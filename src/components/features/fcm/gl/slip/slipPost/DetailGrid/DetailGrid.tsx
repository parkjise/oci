...
const DetailGrid: React.FC<DetailGridProps> = ({
  ...
  const rowData = propRowData || searchData;
  ...
  // 원천Key 더블클릭 핸들러
  const handleSourceKeyDoubleClick = (
    slipExptnSrc: string,
    sourceKey: string,
    rowData: SlipPostSearchResponse
  ) => {
  ...
  // 컬럼 정의
  const columnDefs: ColDef<SlipDataWithStatus>[] = [
    {
      ...createCheckboxColumn<SlipDataWithStatus>("rowNum", "No.", 80, "left"),
      valueGetter: (params) => {
        ...
        <FormAgGrid<SlipDataWithStatus & { id?: string }>
          ...
          gridOptions={{
            ...
            isRowSelectable: (params) => {
              // magamTag가 "Y"이면 선택 불가능 (체크박스 비활성화)
              return params.data?.magamTag !== "Y";
            },
