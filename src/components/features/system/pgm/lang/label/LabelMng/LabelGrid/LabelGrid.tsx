...
import { LabelGridStyles, GridContainer } from "./LabelGrid.styles";
...
// ============================================================================
const StatusCellRenderer: React.FC<ICellRendererParams<LabelDto & { id?: string; chk?: boolean }>> = ({ value }) => {
  const status = value || "";
...
interface LabelGridProps {
  className?: string;
  rowData: (LabelDto & { id?: string; chk?: boolean })[];
...
export interface LabelGridRef {
  getGridData: () => (LabelDto & { id?: string; chk?: boolean })[];
  getSelectedRows: () => (LabelDto & { id?: string; chk?: boolean })[];
}

// ============================================================================
// Component
// ============================================================================
const LabelGrid = forwardRef<LabelGridRef, LabelGridProps>(({
  className,
  rowData,
  langTypeList,
  onModify,
  ...
  useImperativeHandle(ref, () => ({
    getGridData: () => {
      ...
      // 그리드에서 현재 데이터 가져오기
      const allRows: (LabelDto & { id?: string; chk?: boolean })[] = [];
    ...
    getSelectedRows: () => {
      if (!gridApi) return [];
      return gridApi.getSelectedRows() as (LabelDto & { id?: string; chk?: boolean })[];
    },
  }), [gridApi, rowData]);
  ...
  const handleCellValueChanged = useCallback((event: CellValueChangedEvent) => {
    ...
    const rowData = event.data as LabelDto & { id?: string; chk?: boolean };
  ...
  return (
    <LabelGridStyles className={className}>
      {/* 그리드 (공통 툴바 포함 - 전체건수 및 저장버튼 자동 표시) */}
      <GridContainer>
        <AgGrid<LabelDto & { id?: string; chk?: boolean }>
          height="100%"
          columnDefs={columnDefs}
          rowData={rowData || []}
          // 다국어관리 화면은 페이징 없이 전체 데이터를 한 번에 보여준다.
          // FormAgGrid 기본값(pagination: true)을 오버라이드한다.
          pagination={false}
          // 그리드 공통 툴바 사용 (전체건수, 입력, 삭제, 저장 버튼)
          showToolbar={true}
          onAddRow={onAddRow}
